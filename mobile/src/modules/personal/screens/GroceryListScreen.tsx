/**
 * GroceryListScreen — Grocery list: name, quantity, price, optional reminder
 * date, link, notes, and a completed tick. Reminders are scheduled locally
 * via notificationService for items that have a date.
 */
...
import { usePersonalStore, GroceryItem } from '../store';
...
export default function GroceryListScreen() {
  ...
  const { groceryItems, addGroceryItem, updateGroceryItem, deleteGroceryItem, toggleGroceryItem } = usePersonalStore();
  const { user } = useAuth();
  ...
  const pending = groceryItems.filter((i) => !i.completed);
  const done = groceryItems.filter((i) => i.completed);
  const totalPaise = pending.reduce((s, i) => s + (i.price || 0), 0);
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      updateGroceryItem(editingId, fields, user?.id);
    } else {
      addGroceryItem(fields, user?.id);
    }
    resetForm();
  ...
  const renderItem = (item: GroceryItem) => (
    <View key={item.id} style={[styles.itemCard, item.completed && styles.itemCardDone]}>
      <TouchableOpacity style={styles.tickBtn} onPress={() => toggleGroceryItem(item.id, user?.id)}>
        <Ionicons
        ...
        <View style={styles.metaRow}>
          {item.quantity ? (
            <View style={styles.metaBadge}>
              <Ionicons name="scale-outline" size={11} color={colors.primary} />
      ...
      <View style={styles.itemActions}>
        ...
        <TouchableOpacity
          onPress={() => {
            deleteGroceryItem(item.id, user?.id);
          }}
          style={{ padding: 4 }}
        >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.appBar}>
        ...
        <Text style={styles.logoText}>Grocery List</Text>
        <TouchableOpacity
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {groceryItems.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>Grocery list is empty</Text>
            <Text style={styles.emptySubtext}>Tap + to add items with quantity and price</Text>
          </View>
        )}

        {pending.length > 0 && totalPaise > 0 && (
          <View style={styles.totalCard}>
            <Text style={styles.totalLabel}>ESTIMATED TOTAL ({pending.length} items)</Text>
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{editingId ? 'Edit Item' : 'Add Grocery Item'}</Text>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Brand, aisle, substitution..."
                placeholderTextColor={colors.outline}
                value={notes}
