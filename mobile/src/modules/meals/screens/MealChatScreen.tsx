...
export default function MealChatScreen() {
  ...
  const send = async (rawText: string) => {
    ...
    setLoading(true);
    try {
      // Include prior turns so follow-ups like "add another item" work without
      // the user having to repeat or manually enter nutrition values.
      const conversation = messages
        .filter((m) => m.text.trim())
        .slice(-8)
        .map((m) => ({ role: m.role, content: m.text }));
      const result = await analyzeMealManage(text, buildTodayContext(), undefined, conversation);
